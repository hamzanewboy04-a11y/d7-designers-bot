...
async def cmd_start(message: Message, db: Database, config: Config) -> None:
    ...
    if designer:
        ...
        if designer.role == "designer":
            ...
            role_hint = (
                "<b>Что вы можете делать здесь:</b>\n"
                "• отправлять отчёты отзовиков\n"
                "• следить за своим профилем\n"
                "• понимать, что произойдёт после отправки отчёта\n\n"
                "<b>Как это работает:</b>\n"
                "Ваш отчёт уходит на проверку менеджеру. После подтверждения он попадает в выплату."
            )
...
async def cmd_help(message: Message, db: Database, config: Config) -> None:
    ...
    if not designer:
        ...
        text = (
            ...
            "После отправки отчёты попадают в систему выплат. Статус зависит от проверки и обработки администратором.\n\n"
            "<b>Если ошиблись:</b>\n"
        ...
        text = (
            ...
            "<b>Как работает сценарий отзовика:</b>\n"
            "1. Вы отправляете отчёт\n"
            "2. менеджер проверяет его\n"
            "3. После подтверждения отчёт попадает в пачка выплат на выплату\n\n"
            "<b>Важно:</b>\n"
            "Статусы могут быть отправлен на проверку, подтверждён или отклонён. Если отчёт отклонён, обычно нужен комментарий или исправление."
        )
    elif designer.role == "smm":
        text = (
            "<b>❓ Помощь для SMM</b>\n\n"
        ...
        text = (
            ...
            "<b>Что доступно:</b>\n"
            "• reviewer-очередь\n"
            "• создание и обработка пачка выплат'ей\n"
            "• SMM-назначения\n"
